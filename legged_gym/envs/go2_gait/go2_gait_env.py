...
class Go2GaitEnv(LeggedRobot):
    ...
    def _init_foot_periodicity_buffer(self):
        ...
        self.init_foot_thetas = torch.as_tensor(self.cfg.foot_periodicity.init_foot_thetas_list, dtype = torch.float, device = self.device)
        ...
        self.foot_thetas = torch.zeros(size = (self.num_envs, 4), dtype = torch.float, device = self.device)# Clock input shift
        ...
        print('The initial theta vector of gaits:', self.init_foot_thetas)
    ...
    def _reset_foot_periodicity(self, env_ids, calculate_from_slip_model = True, add_noise = False):
        ...
        self.foot_thetas[env_ids, :] = torch.index_select(
            self.init_foot_thetas,
            dim = 0,
            index = torch.randint(low = 0, high = self.init_foot_thetas.size(0), size = (len(env_ids),), device = self.device)
        )
    ...
    def reset_idx(self, env_ids):
        ...
        self._reset_foot_periodicity(env_ids, calculate_from_slip_model = self.foot_periodicity_cfg.calculate_from_slip_model, add_noise = self.foot_periodicity_cfg.add_noise)
    ...
    def _reward_foot_periodicity(self):
        ...
        foot_frcs = torch.norm(self.contact_forces[:, [3, 7, 13, 17], :], dim = -1) # Use calf indices rather than foot indices 
        ...
        R_foot_spds = torch.sum((1 - torch.exp(-2 * foot_spds)) * self.E_C_spd, dim = -1)
